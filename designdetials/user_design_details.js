...
//       console.error("⚠️ FIGMA_ACCESS_TOKEN not found in environment variables");
//       return [];
//     }
...
// // Generate Figma screenshot URL for API (NOT embed)
// function generateFigmaImageUrl(figmaUrl, nodeId = null) {
//   const fileKey = extractFigmaFileKey(figmaUrl);
//   if (!fileKey) return null;
  
//   // If nodeId provided, generate node-specific URL
//   if (nodeId) {
//     return `https://api.figma.com/v1/images/${fileKey}?ids=${nodeId}&format=png&scale=2`;
//   }
  
//   // Otherwise return file-level URL (will get first frame)
//   return `https://api.figma.com/v1/images/${fileKey}?format=png&scale=2`;
// }
...
// // ===== SECURE: Generate screenshot using Figma API Images (NOT Puppeteer) =====
// // ===== SECURE: Generate screenshot using Figma API Images =====
// async function generateFigmaScreenshot(figmaUrl, nodeId, uniqueId, userId, layerOrder = null) {
//   try {
//     const fileKey = extractFigmaFileKey(figmaUrl);
//     const FIGMA_ACCESS_TOKEN = process.env.FIGMA_ACCESS_TOKEN;
...
//     const suffix = layerOrder !== null ? `_layer_${layerOrder}` : '_preview';
//     console.log(`📸 Generating Figma screenshot for ${uniqueId}${suffix}...`);

//     // ✅ FIX: For main preview without nodeId, skip API call
//     if (!nodeId) {
//       console.log("⚠️ No nodeId for main preview, skipping...");
//       return null;
//     }
...
//     const fileName = `${userId}/${uniqueId}${suffix}.jpg`;
...
//     // Generate shareable link (NO embed_url stored)
...
//       embed_url: null, // ❌ NOT storing embed URLs
...
//         if (layers.length > 0) {
//           // Store layer metadata (NO embed URLs)
//           const layerData = layers.map(layer => ({
//             submission_id: submission.id,
//             user_id: user_id,
//             layer_name: layer.layer_name,
//             layer_order: layer.layer_order,
//             layer_embed_url: null, // ❌ NOT storing
...
//             console.log(`✅ Stored ${layers.length} layers`);
...
//                 console.log(`✅ Layer ${layer.layer_order} screenshot stored`);
//               }
//             }
//           }
//         }

//         // Generate main preview screenshot
//         const mainScreenshotPath = await generateFigmaScreenshot(
//           original_url,
//           null,
//           unique_id,
//           user_id
...
//           await supabase_connect
//             .from("design_submissions")
//             .update({ preview_thumbnail: mainScreenshotPath })
...
// // Generate Figma screenshot using Figma API Images
// async function generateFigmaScreenshot(figmaUrl, nodeId, uniqueId, userId, layerOrder = null) {
//   try {
//     const fileKey = extractFigmaFileKey(figmaUrl);
//     const FIGMA_ACCESS_TOKEN = process.env.FIGMA_ACCESS_TOKEN;
...
//     if (!nodeId) {
//       console.log("⚠️ No nodeId provided, skipping screenshot...");
//       return null;
//     }
...
//     // Prepare submission data (NO original_url or embed_url exposed)
//     const submissionData = {
//       user_id,
//       unique_id,
//       design_type,
//       original_url: design_type === 'figma' ? original_url : null, // Stored but never exposed
//       pdf_file_path: design_type === 'pdf' ? pdf_file_path : null,
//       company_name,
//       position,
//       status,
...
//       fetchFigmaLayers(original_url).then(async (layers) => {
//         if (layers.length > 0) {
//           // Store layer metadata with preview URLs only
//           const layerData = layers.map(layer => ({
//             submission_id: submission.id,
//             user_id: user_id,
//             layer_name: layer.layer_name,
...
//           }
//         } else {
//           console.log("⚠️ No layers found in Figma file");
//         }
//       }).catch(error => {
...
//       submission: {
//         ...submission,
//         original_url: undefined // Remove from response
//       },
//       shareable_link: shareableLink,
//       design_type: design_type
//     });
